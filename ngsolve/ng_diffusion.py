...
class diffusion(SecondOrderEllipticCoefficientPDE):
    ...
    def __init__(self, domain, sol_domain,bdr_val=None):
        super().__init__(domain, sol_domain,bdr_val=bdr_val)
...
bc = codomain.from_ngs( ngs.CF(0) )

op = diffusion(domain,codomain,bdr_val=bc)
...
landweber = Landweber(setting, init,data, stepsize=5)
stoprule = (
