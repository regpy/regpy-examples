...
class reaction(SecondOrderEllipticCoefficientPDE):

    def __init__(self, domain, sol_domain):
    ...
    def _bf_0(self):
       return ngs.grad(self.u)*ngs.grad(self.v) * ngs.dx
...
landweber = Landweber(setting, data, init, stepsize=11)
stoprule = (
        rules.CountIterations(2000) +
