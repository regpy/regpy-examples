...
grid_codomain_3=VectorSpace((N, N))
...
power=SquaredModulus(grid_codomain_2)
trace=summation(power.codomain, grid_codomain_3)
ReIm_op=ReIm(grid, grid_domain)
op=trace*power*Tau_op*Mat_op
...
data=np.sum(intensities**2, axis=0)/N_frame

exact_data=op(contrast)
