...
def farfield_matrix(bd, dire, kappa, weight_sl, weight_dl):
    ...
    -------
     ...
     for l, meas in enumerate(dire):
         FFmat[l,:] = 2*np.pi/(np.size(bd.z,1)*np.sqrt(8*np.pi*kappa))*np.exp(-1j*np.pi/4)\
         *(weight_dl*kappa*meas.dot(bd.normal)+1j*weight_sl*bd.zpabs)*np.exp(-1j*kappa*(meas.dot(bd.z)))
